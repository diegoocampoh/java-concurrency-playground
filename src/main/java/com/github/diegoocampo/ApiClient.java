...
//		System.out.println("Parallel stream over Sync Done in "+ duration + " msecs");
		...
		List<Double> prices = client.findPricesASyncTimeOut("ipad");
	...
	public List<Double> findPricesASyncTimeOut(String prod) {
		List<CompletableFuture<Double>> futures = asyncStores.stream()
				.map(store -> store.getPriceAsyncTimeout(prod))
				.collect(toList());
